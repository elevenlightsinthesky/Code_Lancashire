...
// Adding in the suggested questions functionality
const ChatInterface = ({ companyContext, chatInput, setChatInput }) => {
  ...
  const handleSendMessage = async () => {
    try {
      ...
      if (response.ok) {
        setChatHistory([...chatHistory, { user: chatInput, bot: data.message }]); // Add both user and bot messages
  ...
  return (
    <div className="chat-box bg-white p-4 border border-gray-200 rounded-lg shadow-sm">
      <div className="chat-history mb-4">
        {chatHistory.map((entry, index) => (
          <div key={index}>
            <p className="text-gray-800"><strong>User:</strong> {entry.user}</p>
            <p className="text-gray-600"><strong>Bot:</strong> {entry.bot}</p>
          </div>
        ))}
      </div>
      <textarea 
